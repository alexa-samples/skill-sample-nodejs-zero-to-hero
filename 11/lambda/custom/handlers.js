...
const AvailableProductsIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        const response = await monetizationClient.getInSkillProducts(locale);
        // We now need to filter this to find the ISP products that are available for purchase
        const purchasableProducts = logic.getPurchasableProducts(response.inSkillProducts);
        ...
        if (purchasableProducts.length > 0) {
            ...
            speechText = `Products available for purchase at this time are ${logic.getSpeakableListOfProducts(purchasableProducts)}. 
