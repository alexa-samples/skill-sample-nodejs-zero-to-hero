...
module.exports = {
    ...
    getPurchasableProducts(inSkillProductList) {
        console.log('Function: getPurchasableProducts');
        return inSkillProductList.filter(record => record.purchasable === 'PURCHASABLE');
    },
    getSpeakableListOfProducts(entitledProductsList) {
    ...
    async getHoroscopeData(day, month, year) {
        const sign = horoscope.getSign({month: Number(month), day: Number(day)});
        const zodiac = horoscope.getZodiac(Number(year));

        const getAllHoroscopeAsync = (sign) => 
            new Promise(resolve => 
            aztroJs.getAllHoroscope(sign, (res) => resolve(res)));
            
        const result = await getAllHoroscopeAsync(sign);
        result.sign = sign;
        result.zodiac = zodiac;
        //speechText += JSON.stringify(result.today.description).replace(/["]+/g, '');
        return result;
    }
}
